...
import by.gladyshev.util.GreyMinMaxContext;

import java.awt.*;

abstract class Filter {
    //Большинство фильтров в этой лабе не отличаются алгоритмом фильтрации
    //Только маской
    //Поэтому я вынес всю логику фильтрации в класс родитель
    //В большинстве классов наследников только их инициализация и создание маски фильтрации
    protected int[][] mask;
    ...
    protected Color calculateF1(Color[][] underMask, int i, int j){
       ...
       /* if(is==0&&js==0)
        {
            System.out.println();
            System.out.println("undermask");
            for (int k = 0; k < 3; k++) {
                for (int l = 0; l < 3; l++) {
                    System.out.print(underMask[k][l].getBlue()+"  ");
                }
                System.out.println("");
            }
            ...
            System.out.println(GreyMinMaxContext.getMax());
            System.out.println(GreyMinMaxContext.getMin());
        ...
        if(grey<0)
        {
           // System.out.println("alert abs"+(is+1)+" "+(js+1)+" "+grey);
            grey = Math.abs(grey);
        }
        if(grey>255)
        {
            //System.out.println("alert >255 "+(is+1)+" "+(js+1)+" "+grey);
            grey = 255;
        }
